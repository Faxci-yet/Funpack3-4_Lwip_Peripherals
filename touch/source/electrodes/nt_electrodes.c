...
uint32_t _nt_electrode_delta_normalization_process(const struct nt_electrode_data *electrode, int32_t signal)
{
    NT_ASSERT(electrode != NULL);
    if( signal == 0 )
    {
    ...
    {
        if (electrode->rom->delta_multiplier < (0xFFFFU / signal))
        {
